...
Class Master extends DBConnection {
    ...
    $qry = $conn->query("UPDATE `account_list` SET 
        `name` = '{$_POST['name']}', 
    ...
    // Insert new account
    $qry = $conn->query("INSERT INTO `account_list` 
        (`name`, `description`, `credit_limit`, `outstanding_balance`, `status`) 
        VALUES 
        ('{$_POST['name']}', '{$_POST['description']}', '{$_POST['credit_limit']}', '{$_POST['outstanding_balance']}', '{$_POST['status']}')");
}
		return json_encode($resp);
	}
 ...
 function save_invoice() {
    ...
    if (isset($id) && !empty($id)) {
        ...
        $qry = $this->conn->query("UPDATE `invoices` SET 
        ...
        // Insert new invoice
        $qry = $this->conn->query("INSERT INTO `invoices` 
            (`invoice_number`, `account_id`, `date_issued`, `due_date`, `amount`) 
            VALUES 
            ('$invoice_number', '$account_id', '$date_issued', '$due_date', '$amount')");
    }
